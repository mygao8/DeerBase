 ...
 */
public class BTreeScanTest extends DeerBaseTestBase {
    ...
    private void validateScan(int[] columnSizes, int[] rowSizes)
    	...
    	for (int columns : columnSizes) {
            ...
            for (int rows : rowSizes) {
                ...
                BTreeScan scan = new BTreeScan(tid, f.getFileId(), "table", null);
    ...
    @Test public void testRewindPredicates() throws IOException, DbException, TransactionAbortedException {
        ...
        BTreeScan scan = new BTreeScan(tid, f.getFileId(), "table", ipred);
        scan.open();
        ...
        scan = new BTreeScan(tid, f.getFileId(), "table", ipred);
        scan.open();
        for (int i = 0; i < tuplesFiltered.size(); ++i) {
            assertTrue(scan.hasNext());
            Tuple t = scan.next();
