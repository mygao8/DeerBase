...
public class BTreeFileDeleteTest extends DeerBaseTestBase {
	...
	public void testMergeLeafPages() throws Exception {
		...
		BTreePageId rootPtrId = BTreeRootPtrPage.getId(threeLeafPageFile.getFileId());
	...
	public void testDeleteRootPage() throws Exception {
		...
		BTreePageId rootPtrId = BTreeRootPtrPage.getId(twoLeafPageFile.getFileId());
		BTreeRootPtrPage rootPtr = (BTreeRootPtrPage) Database.getBufferPool().getPage(
				tid, rootPtrId, Permissions.READ_ONLY);
	...
	public void testReuseDeletedPages() throws Exception {
		...
		for(int i = 0; i < 502; ++i) {
			Database.getBufferPool().insertTuple(tid, threeLeafPageFile.getFileId(),
	...
	public void testRedistributeInternalPages() throws Exception {
		...
		BTreeRootPtrPage rootPtr = (BTreeRootPtrPage) Database.getBufferPool().getPage(
				tid, BTreeRootPtrPage.getId(bf.getFileId()), Permissions.READ_ONLY);
		BTreeInternalPage root = (BTreeInternalPage) Database.getBufferPool().getPage(
				tid, rootPtr.getRootId(), Permissions.READ_ONLY);
		assertEquals(502, root.getNumEmptySlots());
	...
	public void testDeleteInternalPages() throws Exception {
		...
		BTreeRootPtrPage rootPtr = (BTreeRootPtrPage) Database.getBufferPool().getPage(
				tid, BTreeRootPtrPage.getId(bigFile.getFileId()), Permissions.READ_ONLY);
		BTreeInternalPage root = (BTreeInternalPage) Database.getBufferPool().getPage(
				tid, rootPtr.getRootId(), Permissions.READ_ONLY);
		...
		// replaced the root
		BTreePageId rootPtrId = BTreeRootPtrPage.getId(bigFile.getFileId());
