...
import deerBase.systemtest.DeerBaseTestBase;
...
public class BTreeFileInsertTest extends DeerBaseTestBase {
	...
	public void testSplitLeafPage() throws Exception {
		...
		// now insert a tuple
		Database.getBufferPool().insertTuple(tid, onePageFile.getFileId(), BTreeUtility.getBTreeTuple(5000, 2));
		...
		BTreePageId rootPtrPid = new BTreePageId(onePageFile.getFileId(), 0, BTreePageId.ROOT_PTR);
	...
	public void testSplitRootPage() throws Exception {
		...
		// now insert a tuple
		Database.getBufferPool().insertTuple(tid, bigFile.getFileId(), BTreeUtility.getBTreeTuple(10, 2));

		// there should now be 505 leaf pages + 3 internal nodes
		assertEquals(508, bigFile.numPages());

		// the root node should be an internal node and have 2 children (1 entry)
		BTreePageId rootPtrPid = new BTreePageId(bigFile.getFileId(), 0, BTreePageId.ROOT_PTR);
		BTreeRootPtrPage rootPtr = (BTreeRootPtrPage) Database.getBufferPool().getPage(tid, rootPtrPid, Permissions.READ_ONLY);
		BTreePageId rootId = rootPtr.getRootId();
		assertEquals(rootId.pgcateg(), BTreePageId.INTERNAL);
		BTreeInternalPage root = (BTreeInternalPage) Database.getBufferPool().getPage(tid, rootId, Permissions.READ_ONLY);
...
//			System.out.println("splitRoot | " + i + "-th insert " + t.toString());
			Database.getBufferPool().insertTuple(tid, bigFile.getFileId(), t);
	...
	public void testSplitInternalPage() throws Exception {
		...
		for(int i = 0; i < 100; i++) {
			int item = rand.nextInt(BTreeUtility.MAX_RAND_VALUE);
			Tuple t = BTreeUtility.getBTreeTuple(item, 2);
//			System.out.println("splitInternal | " + i + "-th insert " + t.toString());
			Database.getBufferPool().insertTuple(tid, bigFile.getFileId(), t);
